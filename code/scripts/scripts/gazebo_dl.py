...
from deep_networks import Classifier, Regressor, CNNClassifier, CNNRegressor, LaserScanDataset
                ...
                if scenario is not None:
                    ...
                    if not self.gazebo_launch._shutting_down:
                        ...
                        try:
                            ...
                            result = test_driver.run_test_dl(goal_pose=scenario.getGoal(), models=self.models,
                                                             predict_recorder=predict_recorder,
                                                             ranges=self.ranges, suffix=self.suffix,
                                                             density=scenario.density,
                                                             truth=self.best_configs)
...
def test(read_path, ranges, save_path, suffix='gt', cnn=False, checkpoint=None, env=None, pickleload=None, id=0):
    ...
    if os.path.isfile(pickleload):
        ...
        for param in checkpoints.keys():
            ...
            if suffix == 'regressor':
                if cnn:
                    model = CNNRegressor(3, 128)
                else:
                    model = Regressor(256)
            else:
                if cnn:
                    model = CNNClassifier(3, 128, ckpt['num_labels'])
                else:
                    model = Classifier(256, ckpt['num_labels'])
        ...
        # '''
        param = 'max_depth'  # max_depth, planner_frequency
        dataset = LaserScanDataset(training_data[param]['X'], training_data[param]['Y'])
        dataloader = DataLoader(dataset, batch_size=4096)
        if suffix == 'regressor':
            if cnn:
                model = CNNRegressor(3, 128, output=1 + (param == 'combined'))
            else:
                model = Regressor(256, output=1 + (param == 'combined'))
            loss_fn = nn.MSELoss()
        else:
            if cnn:
                model = CNNClassifier(3, 128, len(ranges[param]))
            else:
                model = Classifier(256, len(ranges[param]))
        ...
        for epoch in range(EPOCHS):
            ...
            if epoch % 3 == 0:
                ...
                if hasattr(model, 'num_labels'):
                    save_dict['num_labels'] = model.num_labels
    ...
    master = MultiMasterCoordinator(result_recorders=result_recorders, gazebo=GazeboDL, model=models, ranges=ranges,
                                    best_configs=best_configs, path=save_path + 'model/' + model_name + '/',
                                    suffix=suffix)
    master.start()
    tasks = []
...
def print_results(save_path, suffix=['classifier', 'regressor'], seed=None):
    ...
    default_result = find_results(save_path + 'default/' + task["robot"] + '_' + task["controller"] +
                                  '_' + str(task["min_obstacle_spacing"]) + '/', task['params'], seed)
    print('default:')
...
def predict():
    ...
    for env in ['fourth_floor']:
        ranges = {'planner_frequency': values, 'max_depth': depth_values,}  # 'planner_frequency': values, 'max_depth': depth_values,
        print('\n' + env)
        for id in range(5):
            print(id)
