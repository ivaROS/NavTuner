...
class ResultRecorder(object):
    ...
    def twistCB(self, data):
        ...
        # print(self.scan is not None, self.egocircle is not None, self.plan is not None, self.image is not None, self.depth is not None, self.feedback is not None)
        if (
                self.scan is not None and self.egocircle is not None and self.plan is not None and self.image is not None and self.depth is not None and self.feedback is not None):
...
def run_test(goal_pose):
    ...
    return {'result': result, 'time': task_time, 'path_length': path_length,
            'bag_file_path': result_recorder.bagfilepath}
